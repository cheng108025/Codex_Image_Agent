...
> Tier C — 遠征士兵（有名字的犧牲者）。本檔為文字版身份規格，供 `00-dave-character-identity-anchor.png` 生成之依據。
...
- 故事時期／服裝版本：荒野戰場遠征期 — 赫克托制式步兵甲（`CANON`）
...
- 主武器：制式短劍（standard-issue short sword）
...
- 腰部：制式腰帶，佩制式短劍（`DESIGN-PROPOSAL`）
- 下襬／披風：無披風或僅短布甲裙（`DESIGN-PROPOSAL`）
- 腿部／鞋靴：制式護脛、軍靴（`DESIGN-PROPOSAL`）
- 背面結構：制式甲背片，無特殊構造（`DESIGN-PROPOSAL`）

## 色票與材質
- 主色：赫克托軍制式配色——鋼灰與暗金／深色系（`DESIGN-PROPOSAL`；與全書金／銀藍／深色系一致）
- 輔色：深棕皮革、布料內襯（`DESIGN-PROPOSAL`）
- 金屬：制式甲片鋼、扣件（`DESIGN-PROPOSAL`）
- 寶石：無（`DESIGN-PROPOSAL`）
- 布料：制式軍服布料（`DESIGN-PROPOSAL`）
- 皮革：護臂、腰帶、軍靴（`DESIGN-PROPOSAL`）
- 特殊材質：無

## 道具與特殊構造
- 武器：制式步兵短劍或斧（`DESIGN-PROPOSAL`）
- 飾品：無（刻意樸素）
