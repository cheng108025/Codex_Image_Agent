# 戴夫 Dave — Image Generation Prompts（FULL v2 package: 00–10）
...
> **簽名武器 PENDING-USER-INPUT（貫穿 09）**：文檔僅載「短劍或斧」二選一。本檔 09 道具表推薦**短劍**（理由見 CHARACTER_SPEC.md），但正式標記 `PENDING-USER-INPUT`，待使用者決定；此分歧僅影響 09，不阻擋 00–08／10 的生成與驗收。

> **兩階段規則（強制）**：第一階段只生成 `00-dave-character-identity-anchor.png`（三視圖：A-pose 正面全身＋正面頭肩＋45° 三分之四頭肩，缺一即 REJECT 重生成），生成後停止等待使用者核准。核准後 00 成為 01–10 的 Image 1 身份／服裝唯一正本。三張 style 圖永遠只控制畫風。
...
| 編號 | 檔案 | 內容 |
| --- | --- | --- |
| 00 | `00-dave-character-identity-anchor.png` | 身份母圖：A-pose 正面全身＋正面頭肩＋45° 頭肩，零道具 |
| 01 | `01-dave-front-fullbody.png` | 正面全身，A-pose，零道具 |
| 02 | `02-dave-left-profile-fullbody.png` | 相機位於角色解剖學左側全身，A-pose，零道具 |
| 03 | `03-dave-right-profile-fullbody.png` | 相機位於角色解剖學右側全身，A-pose，零道具 |
| 04 | `04-dave-back-fullbody.png` | 背面全身，A-pose，零道具 |
| 05 | `05-dave-four-view-master.png` | 四視圖總表（正面→左→右→背），零道具 |
| 06 | `06-dave-expression-sheet.png` | 八表情臉部表，4×2 網格 |
| 07 | `07-dave-costume-detail-sheet.png` | 服裝細節表（含右側腰際佩劍掛點） |
| 08 | `08-dave-color-material-sheet.png` | 色票／材質表（已補固定 hex） |
| 09 | `09-dave-prop-construction-sheet.png` | 道具表：短劍（推薦方案，`PENDING-USER-INPUT`；全書唯一出現位置） |
| 10 | `10-dave-body-reference-sheet.png` | 素體參考圖（SMPL-X／人體擬合用） |

---

## 00 — `00-dave-character-identity-anchor.png`（STAGE 1 — 只先生成這張，停止等核准）
...
Body metrics:
Height 175 cm. Approximately 6.8 heads tall (design proportion, stockier build). Shoulder width approximately 2.6 head-widths (the sturdiest build of the pair). Fully symmetric build visually on this zero-prop sheet; his signature weapon (still undecided) will be mounted at the right hip in sheet 09 only.
...
Negative prompt:
photorealistic, 3D render, western comic style, chibi, super-deformed, thick outlines, harsh lineart, sketchy unfinished lines, flat unshaded colors, muddy colors, excessive bloom, dramatic cinematic background, scenery, extreme perspective, fisheye, cropped body, inconsistent anatomy, different face, different hairstyle, costume redesign, random accessories, duplicate character, extra limbs, extra fingers, text, labels, letters, numbers, logo, signature, watermark, jpeg artifacts, low resolution, blurry, heavy hero armor, ornate heroic plate armor, wings, horns, tail, crown, cape, officer sash, command sash, gilded decoration, tall slender build, lanky build, gemstones, ornate gemstones, shining jewels, gold pauldrons, gilded pauldrons, holding weapon, weapon in hand, hand on sword pommel, arms touching torso, arms pressed against body, contrapposto, action pose, heroic pose, battle stance, T-pose, bent elbows, uneven shoulders, crossed legs, hands on hips, crossed arms, dynamic pose, walking, running, cast shadows, strong highlights, rim light, dramatic lighting, weapon, sword, axe, scabbard, sheath, staff, shield, prop object, reference character face, copying any style-image character, blending the three reference characters, Kritz black-and-gold pauldrons, shoulder chains, purple gemstones, Revanas ice-crystal horns, crystalline wings
...
## 01 — `01-dave-front-fullbody.png`（STAGE 2 — 正面全身）
...
Input images:
- Image 1: the approved 00-dave-character-identity-anchor.png and sole identity/costume authority.
...
Identity invariants:
Identical character from Image 1: same plain unremarkable common-soldier face, very short dark cropped hair (#2B211A), light hazel/gray-brown eyes (#8A7256), healthy weathered skin (#E8C09E).
...
## 02 — `02-dave-left-profile-fullbody.png`（STAGE 2 — 相機位於角色解剖學左側）
...
Negative prompt:
photorealistic, 3D render, western comic style, chibi, super-deformed, thick outlines, harsh lineart, sketchy unfinished lines, flat unshaded colors, muddy colors, excessive bloom, dramatic cinematic background, scenery, extreme perspective, fisheye, cropped body, inconsistent anatomy, different face, different hairstyle, costume redesign, random accessories, duplicate character, extra limbs, extra fingers, text, labels, letters, numbers, logo, signature, watermark, jpeg artifacts, low resolution, blurry, heavy hero armor, ornate armor, gemstones, gold pauldrons, wings, horns, tail, torch, holding weapon, weapon in hand, weapon, sword, axe, scabbard, sheath, staff, shield, prop object, reference character face, copying any style-image character, arms touching torso, arms pressed against body, contrapposto, action pose, heroic pose, battle stance, T-pose, bent elbows, uneven shoulders, crossed legs, hands on hips, crossed arms, dynamic pose, walking, running, cast shadows, strong highlights, rim light, dramatic lighting, front view, back view, three-quarter view, head turned toward viewer, camera on anatomical right side
...
## 03 — `03-dave-right-profile-fullbody.png`（STAGE 2 — 相機位於角色解剖學右側）
...
Costume invariants:
Identical attire from Image 1 seen from the anatomical-right camera position: matte-steel cuirass silhouette (#5F6368), shoulder guard and bracer in profile (#3D2E22), plain regulation belt with an empty mount point and NO weapon attached, steel greaves and boots. NO weapon anywhere; the signature weapon (PENDING-USER-INPUT) is reserved exclusively for prop sheet 09.
...
## 04 — `04-dave-back-fullbody.png`（STAGE 2 — 背面全身）
...
Primary request:
Render Dave as ONE full-body BACK view (180 degrees) in true orthographic projection, in the same standard neutral A-pose as the front view: standing upright, feet shoulder-width apart, arms straight and relaxed at approximately 30-45 degrees away from the body, palms open with fingers slightly spread, shoulders level, weight evenly distributed, no contrapposto, no action pose, no T-pose. Head faces exactly away from the viewer.

Identity invariants:
Identical character from Image 1: same very short dark cropped hair seen from behind (#2B211A), same build and proportions.

Body metrics:
Height 175 cm. Approximately 6.8 heads tall. Shoulder width approximately 2.6 head-widths.

Costume invariants:
Identical attire from Image 1 seen from behind: plain regulation back plate (#5F6368), no ornament, shoulder guards symmetric, regulation belt with an empty mount point at the right hip and NO weapon attached, steel greaves and boots seen from behind. NO weapon anywhere; the signature weapon (PENDING-USER-INPUT) is reserved exclusively for prop sheet 09.
...
## 05 — `05-dave-four-view-master.png`（STAGE 2 — 四視圖總表）
...
## 06 — `06-dave-expression-sheet.png`（STAGE 2 — 八表情臉部表）
...
Input images:
- Image 1: use only as the character identity, hairstyle, costume-collar and color reference (the approved 00-dave-character-identity-anchor.png). Ignore its original pose, framing and composition.
...
## 07 — `07-dave-costume-detail-sheet.png`（STAGE 2 — 服裝細節表）
...
4. Waist belt detail, including the empty mount point at the RIGHT hip reserved for the signature weapon (weapon itself NOT shown here — see sheet 09).
...
Costume invariants:
Exact materials and colors as Image 1: matte steel plate (#5F6368), deep-brown leather (#3D2E22), plain regulation cloth (#4A4238). Same construction as Image 1; no redesign; NO weapon anywhere (signature weapon, PENDING-USER-INPUT, belongs to sheet 09 only).
...
- The right-hip belt mount point must be shown EMPTY (no weapon); the weapon itself is PENDING-USER-INPUT and belongs to sheet 09 only.
- No text or watermark. Do not invent missing canonical details.
```

---

## 08 — `08-dave-color-material-sheet.png`（STAGE 2 — 色票／材質表，已補固定 hex）
...
## 09 — `09-dave-prop-construction-sheet.png`（STAGE 2 — 道具表：短劍〔推薦方案，PENDING-USER-INPUT〕，全書唯一出現位置）

> **PENDING-USER-INPUT — 正典衝突警示**：CHARACTER_SPEC.md 記載簽名武器為「短劍或斧」二選一，文檔未定案。本提示詞推薦**短劍（standard-issue short sword）**：短劍更符合「制式」步兵標準化裝備語言，闊斧偏向蠻族／伐木意象，與「平凡、無英雄氣派」的正典基調略有落差；短劍也與同組文森特、鮑曼的短刃形制一致，便於裝備語言統一。**此為推薦而非定案**，若使用者選擇斧，僅需重新生成本張（09），00–08／10 不受影響、無需重生成。
...
Input images:
- Image 1: the approved 00-dave-character-identity-anchor.png — use ONLY to match material language and craftsmanship level; do not depict the character.
...
## 10 — `10-dave-body-reference-sheet.png`（STAGE 2 — 素體參考圖，SMPL-X／人體擬合用）
...
Input images:
- Image 1: the approved 00-dave-character-identity-anchor.png and sole identity authority (face, hair, skin tone, body proportions).
- Images 2-4: shared style references only; do not copy their character identities.
...
- [ ] 左右位置正確：佩劍側鎖定為右側腰際（`DESIGN-PROPOSAL`）；00–05／10 零道具無武器可見，僅 07 顯示空掛點、09 顯示武器本身
...
| 張 | 額外驗收 |
| --- | --- |
| 00 | 三視圖齊全（A-pose 正面全身＋正面頭肩＋45° 頭肩），缺一即 REJECT；零道具（無短劍／斧） |
| 01 | 正面、A-pose、正交 |
| 02 | 相機位於解剖學左側、A-pose、正交（不得只寫「朝左」） |
| 03 | 相機位於解剖學右側、A-pose、正交（不得只寫「朝右」）；武器掛點側雖在此側，仍不得顯示武器 |
| 04 | 背面、A-pose、正交 |
| 05 | 順序正面→左→右→背；四視圖同高度、同尺寸、同比例、頭頂線與腳底線水平對齊 |
| 06 | 恰好 8 格 4×2、順序正確、只有表情改變 |
| 07 | 胸甲拆解面板＋無胸甲軀幹視圖必須存在；右側腰際掛點須顯示為空；無武器本身 |
| 08 | 色票方塊與鎖定 hex 完全一致 |
| 09 | 短劍（推薦方案，`PENDING-USER-INPUT`）唯一出現於此；無角色、無臉、無人物剪影；若使用者改選斧，僅需重生成本張 |
