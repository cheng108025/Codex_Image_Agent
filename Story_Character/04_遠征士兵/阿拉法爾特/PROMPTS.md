# 阿拉法爾特·斯洛柏格 Alafart Sloberg — Image Generation Prompts（新版七張制 01–07）
...
> **兩階段規則（強制）**：第一階段只生成 `01-alafart-front-fullbody.png`（三視圖：A-pose 正面全身＋正面頭肩＋45° 三分之四頭肩，缺一即 REJECT 重生成），生成後停止等待使用者核准。核准後 01 成為 02–07 的 Image 1 身份／服裝唯一正本。畫風輸入一律依 `../../STYLE_ANCHOR.md` 決定。
...
| 編號 | 檔案 | 內容 |
| --- | --- | --- |
| 01 | `01-alafart-front-fullbody.png` | 身份錨點：正面全身，A-pose，零道具（**STAGE 1，先生成並等核准**） |
| 02 | `02-alafart-four-view-master.png` | 四視圖總表（正面→左→右→背），零道具 |
| 03 | `03-alafart-expression-sheet.png` | 表情表 |
| 04 | `04-alafart-costume-detail-sheet.png` | 服裝細節 |
| 05 | `05-alafart-color-material-sheet.png` | 色彩與材質 |
| 06 | `06-alafart-prop-construction-sheet.png` | 道具結構 |
| 07 | `07-alafart-body-reference-sheet.png` | 身體參考（SMPL-X） |

---

## 01 — `01-alafart-front-fullbody.png`（STAGE 1 — 只先生成這張，停止等核准）
...
Identity invariants:
Per the Character definition above (no prior identity image exists; this sheet establishes it): same angular hard-jawed adult male face with short beard/heavy stubble (#1C1A18), dark-brown eyes (#3A2A1E), very short black cropped hair (#1C1A18), weathered tanned skin (#C99A72).
...
## 02 — `02-alafart-four-view-master.png`（STAGE 2 — 四視圖總表：正面→左→右→背）
...
Input images:
- Image 1: the approved 01-alafart-front-fullbody.png and sole identity/costume authority.
...
## 03 — `03-alafart-expression-sheet.png`（STAGE 2 — 八表情臉部表）
...
Input images:
- Image 1: use only as the character identity, hairstyle, beard, costume-collar and color reference (the approved 01-alafart-front-fullbody.png). Ignore its original pose, framing and composition.
- Image 2: if this role is not the active style anchor, use the approved anchor 01 resolved from `../../STYLE_ANCHOR.md` for style only; otherwise omit Image 2.
...
## 04 — `04-alafart-costume-detail-sheet.png`（STAGE 2 — 服裝細節表）
...
## 05 — `05-alafart-color-material-sheet.png`（STAGE 2 — 色票／材質表，已補固定 hex）
...
## 06 — `06-alafart-prop-construction-sheet.png`（STAGE 2 — 道具表：火炬與短刀，全書唯一出現位置）
...
Input images:
- Image 1: the approved 01-alafart-front-fullbody.png — use ONLY to match material language and craftsmanship level; do not depict the character.
- Image 2: if this role is not the active style anchor, use the approved anchor 01 resolved from `../../STYLE_ANCHOR.md` for style only; otherwise omit Image 2.
...
## 07 — `07-alafart-body-reference-sheet.png`（STAGE 2 — 素體參考圖，SMPL-X／人體擬合用）
...
Input images:
- Image 1: the approved 01-alafart-front-fullbody.png and sole identity authority (face, hair, skin tone, body proportions).
- Image 2: if this role is not the active style anchor, use the approved anchor 01 resolved from `../../STYLE_ANCHOR.md` for style only; otherwise omit Image 2.
...
- [ ] 色票未漂移：#3C2E24／#4A3B2E／#52565C／#C99A72／#1C1A18／#3A2A1E／#E8873A（僅 06）
