# 阿拉法爾特·斯洛柏格 Alafart Sloberg — Image Generation Prompts（v2 FULL package: 00 / 05 / 06 / 07 / 08 / 09 / 10）
...
> **兩階段規則（強制）**：第一階段只生成 `00-alafart-character-identity-anchor.png`（三視圖，缺一不可），生成後停止等待使用者核准。核准後 00 成為本檔其餘圖片的 Image 1 身份／服裝唯一正本。三張 style 圖永遠只控制畫風。
...
| 編號 | 檔案 | 內容 |
| --- | --- | --- |
| 00 | `00-alafart-character-identity-anchor.png` | 身份母圖：A-pose 正面全身＋正面頭肩＋45° 三分之四頭肩，零道具 |
...
| 08 | `08-alafart-color-material-sheet.png` | 色票／材質表 |
...
## 05 — `05-alafart-four-view-master.png`（STAGE 2）
...
Identity invariants:
All four figures are the identical character from Image 1: same angular hard-jawed adult male face with short beard/heavy stubble, very short black cropped hair, dark-brown eyes, weathered tanned skin. Head-top line and foot line perfectly horizontally aligned across all four views; silhouettes must be mutually alignable. Neutral A-pose views need not show the angry facial expression; a calm neutral face is acceptable for the orthographic body views.

Body metrics:
Height 178 cm. Approximately seven heads tall, solid sturdy build. Fully symmetric build.
...
Identity invariants:
Keep exactly the same character in all eight portraits. Preserve the identical facial identity, angular hard-jawed face proportions, short beard/heavy stubble, weathered tanned skin, dark-brown eyes, very short black cropped hair, neck proportions, and the visible padded high collar. Use the same framing, camera distance, head size, lighting, color grading and front-facing angle in every panel. Only the facial expression may change; his baseline (neutral panel) still carries a residue of resentment.
...
## 08 — `08-alafart-color-material-sheet.png`（STAGE 2 — 色票／材質表）
...
Primary request:
Create a production color-and-material sheet for Alafart: a medium-size neutral A-pose front figure (character and costume only, no weapon or torch) on the left, and on the right a column of large clean color swatches with material rendering patches for each locked color group, named by color, not hex (CHARACTER_SPEC.md gives no hex values for this character):
...
- weathered tanned skin; very short black hair and beard/stubble; dark-brown eyes
...
Identity invariants:
The reference figure is the identical character from Image 1; same face, hair, proportions (178 cm, seven heads, solid sturdy build).
...
Identity invariants:
Same character as Image 1: same face, very short black cropped hair, short beard/heavy stubble, weathered tanned skin, dark-brown eyes. The body underneath must be consistent with the armored figure of the approved sheets: solid, sturdy, seasoned-infantry build.

Body metrics:
Height 178 cm. Approximately seven heads tall. Fully symmetric. Identical height and proportions in both views; head-top and foot lines horizontally aligned across the two views.
...
- [ ] 髮型、髮色（黑色極短平頭）、鬚型（短鬚／濃鬍渣）一致
- [ ] 身材比例：178cm／約七頭身、結實粗壯
- [ ] 服裝結構與 Costume Lock 一致（層次、配件數量、位置）
- [ ] 左右位置正確（全對稱設計，無鏡像錯亂）
- [ ] 色票未漂移（暗棕—炭黑皮革／暗鋼灰鋼片／深褐眼／黑髮，對照 CHARACTER_SPEC.md）
