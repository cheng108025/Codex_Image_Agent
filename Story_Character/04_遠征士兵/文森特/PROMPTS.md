# 文森特 Vincent — Image Generation Prompts（新版七張制 01–07）
...
> **簽名武器 PENDING-USER-INPUT（貫穿 06）**：文檔僅載「短劍或長矛」二選一。本檔 06 道具結構表推薦**短劍**（理由見 CHARACTER_SPEC.md），但正式標記 `PENDING-USER-INPUT`，待使用者決定；此分歧僅影響 06，不阻擋 01–05／07 的生成與驗收。

> **兩階段規則（強制）**：第一階段只生成 `01-vincent-front-fullbody.png`（三視圖：A-pose 正面全身＋正面頭肩＋45° 三分之四頭肩，缺一即 REJECT 重生成），生成後停止等待使用者核准。核准後 01 成為 02–07 的 Image 1 身份／服裝唯一正本。畫風輸入一律依 `../../STYLE_ANCHOR.md` 決定。
...
| 編號 | 檔案 | 內容 |
| --- | --- | --- |
| 01 | `01-vincent-front-fullbody.png` | 身份錨點：正面全身，A-pose，零道具（**STAGE 1，先生成並等核准**） |
| 02 | `02-vincent-four-view-master.png` | 四視圖總表（正面→左→右→背），零道具 |
| 03 | `03-vincent-expression-sheet.png` | 表情表 |
| 04 | `04-vincent-costume-detail-sheet.png` | 服裝細節 |
| 05 | `05-vincent-color-material-sheet.png` | 色彩與材質 |
| 06 | `06-vincent-prop-construction-sheet.png` | 道具結構 |
| 07 | `07-vincent-body-reference-sheet.png` | 身體參考（SMPL-X） |

---

## 01 — `01-vincent-front-fullbody.png`（STAGE 1 — 只先生成這張，停止等核准）
...
Identity invariants:
Per the Character definition above (no prior identity image exists; this sheet establishes it): same plain unremarkable common-soldier face, dark-brown neat military short hair (#2E2015), dark-brown eyes (#3A2818), healthy weathered skin (#E3BB95).
...
## 02 — `02-vincent-four-view-master.png`（STAGE 2 — 四視圖總表：正面→左→右→背）
...
Input images:
- Image 1: the approved 01-vincent-front-fullbody.png and sole identity/costume authority.
...
## 03 — `03-vincent-expression-sheet.png`（STAGE 2 — 八表情臉部表）
...
Input images:
- Image 1: use only as the character identity, hairstyle, costume-collar and color reference (the approved 01-vincent-front-fullbody.png). Ignore its original pose, framing and composition.
- Image 2: if this role is not the active style anchor, use the approved anchor 01 resolved from `../../STYLE_ANCHOR.md` for style only; otherwise omit Image 2.
...
## 04 — `04-vincent-costume-detail-sheet.png`（STAGE 2 — 服裝細節表）
...
4. Waist belt detail, including the empty mount point at the LEFT hip reserved for the signature weapon (weapon itself NOT shown here — see sheet 06).
...
Costume invariants:
Exact materials and colors as Image 1: matte steel plate (#63676C), deep-brown leather (#3D2E22), plain regulation cloth (#4A4238). Same construction as Image 1; no redesign; NO weapon anywhere (signature weapon, PENDING-USER-INPUT, belongs to sheet 06 only).
...
- The left-hip belt mount point must be shown EMPTY (no weapon); the weapon itself is PENDING-USER-INPUT and belongs to sheet 06 only.
- No text or watermark. Do not invent missing canonical details.
```
...
## 05 — `05-vincent-color-material-sheet.png`（STAGE 2 — 色票／材質表，已補固定 hex）
...
## 06 — `06-vincent-prop-construction-sheet.png`（STAGE 2 — 道具表：短劍〔推薦方案，PENDING-USER-INPUT〕，全書唯一出現位置）


> **PENDING-USER-INPUT — 正典衝突警示**：CHARACTER_SPEC.md 記載簽名武器為「短劍或長矛」二選一，文檔未定案。本提示詞推薦**短劍（regulation short sword）**：長矛屬雙手長柄兵器，與其單兵近戰護衛的敘事定位不完全吻合，且不易與同組角色的短刃裝備並列於 06 道具結構表；短劍也與同組戴夫、鮑曼的短刃形制一致，便於裝備語言統一。**此為推薦而非定案**，若使用者選擇長矛，僅需重新生成本張（06），01–05／07 不受影響、無需重生成。
...
Input images:
- Image 1: the approved 01-vincent-front-fullbody.png — use ONLY to match material language and craftsmanship level; do not depict the character.
- Image 2: if this role is not the active style anchor, use the approved anchor 01 resolved from `../../STYLE_ANCHOR.md` for style only; otherwise omit Image 2.
...
## 07 — `07-vincent-body-reference-sheet.png`（STAGE 2 — 素體參考圖，SMPL-X／人體擬合用）
...
Input images:
- Image 1: the approved 01-vincent-front-fullbody.png and sole identity authority (face, hair, skin tone, body proportions).
- Image 2: if this role is not the active style anchor, use the approved anchor 01 resolved from `../../STYLE_ANCHOR.md` for style only; otherwise omit Image 2.
...
- [ ] 左右位置正確：佩劍側鎖定為左側腰際（`DESIGN-PROPOSAL`）；01–02／07 零道具無武器可見，僅 07 顯示空掛點、06 顯示武器本身
...
| 02 | 順序正面→左→右→背；四視圖同高度、同尺寸、同比例、頭頂線與腳底線水平對齊；相機位於解剖學左側、A-pose、正交（不得只寫「朝左」）；武器掛點側雖在此側，仍不得顯示武器；相機位於解剖學右側、A-pose、正交（不得只寫「朝右」）；背面、A-pose、正交 |
| 03 | 恰好 8 格 4×2、順序正確、只有表情改變 |
| 04 | 胸甲拆解面板＋無胸甲軀幹視圖必須存在；左側腰際掛點須顯示為空；無武器本身 |
| 05 | 色票方塊與鎖定 hex 完全一致 |
| 06 | 短劍（推薦方案，`PENDING-USER-INPUT`）唯一出現於此；無角色、無臉、無人物剪影；若使用者改選長矛，僅需重生成本張 |
