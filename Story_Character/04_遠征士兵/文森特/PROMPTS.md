# 文森特 Vincent — Image Generation Prompts（FULL v2 package: 00–10）

> **用途聲明（必讀）**
> 這套圖片是 **3D 建模參考包（modeling reference pack）**，不是完成的 3D 模型。
> 純 AI img2vid 無法保證跨鏡頭的角色完全一致；正式影片必須先以本參考包建立同一個經核准的 **Blender／VRM 角色模型**，並以該模型作為**唯一角色正本**驅動所有鏡頭。

> **角色版本 ID：`VINCENT-V1-EXPEDITION-INFANTRY`**（赫克托制式步兵甲版；其他時期／服裝需另立版本，不得混用）

> **簽名武器 PENDING-USER-INPUT（貫穿 09）**：文檔僅載「短劍或長矛」二選一。本檔 09 道具表推薦**短劍**（理由見 CHARACTER_SPEC.md），但正式標記 `PENDING-USER-INPUT`，待使用者決定；此分歧僅影響 09，不阻擋 00–08／10 的生成與驗收。

> **兩階段規則（強制）**：第一階段只生成 `00-vincent-character-identity-anchor.png`（三視圖：A-pose 正面全身＋正面頭肩＋45° 三分之四頭肩，缺一即 REJECT 重生成），生成後停止等待使用者核准。核准後 00 成為 01–10 的 Image 1 身份／服裝唯一正本。三張 style 圖永遠只控制畫風。
...
| 編號 | 檔案 | 內容 |
| --- | --- | --- |
| 00 | `00-vincent-character-identity-anchor.png` | 身份母圖：A-pose 正面全身＋正面頭肩＋45° 頭肩，零道具 |
| 01 | `01-vincent-front-fullbody.png` | 正面全身，A-pose，零道具 |
| 02 | `02-vincent-left-profile-fullbody.png` | 相機位於角色解剖學左側全身，A-pose，零道具 |
| 03 | `03-vincent-right-profile-fullbody.png` | 相機位於角色解剖學右側全身，A-pose，零道具 |
| 04 | `04-vincent-back-fullbody.png` | 背面全身，A-pose，零道具 |
| 05 | `05-vincent-four-view-master.png` | 四視圖總表（正面→左→右→背），零道具 |
| 06 | `06-vincent-expression-sheet.png` | 八表情臉部表，4×2 網格 |
| 07 | `07-vincent-costume-detail-sheet.png` | 服裝細節表（含左側腰際佩劍掛點） |
| 08 | `08-vincent-color-material-sheet.png` | 色票／材質表（已補固定 hex） |
| 09 | `09-vincent-prop-construction-sheet.png` | 道具表：短劍（推薦方案，`PENDING-USER-INPUT`；全書唯一出現位置） |
...
Vincent, an ordinary Hector foot-soldier (apparent age mid-20s), one of Gawain's rank-and-file infantrymen. Build: tall and plainly proportioned, roughly 7.2 heads, height LOCKED at about 185cm — the "tall one" of a tall-and-sturdy pair. Plain, unremarkable, brave common-soldier face with NO heroic aura — deliberately ordinary and honest. Locked identity defaults: dark-brown neat military short hair suited to wearing a helmet (#2E2015), dark-brown eyes with an earnest steady gaze (#3A2818), healthy weathered skin tone (#E3BB95). Costume: standard-issue Hector infantry armor — cloth/leather inner layer (#4A4238) under regulation matte-steel plate pieces (#63676C), plain regulation steel gorget, standard steel shoulder guards with NO officer sash or ornament, a plain matte-steel breastplate bearing a small low-key embossed Hector army insignia (a simple geometric mark in muted dark-gold #7A6A42, no gemstones), deep-brown leather bracers (#3D2E22), a regulation leather belt with an empty mount point at the left hip and NO weapon attached (his signature weapon is reserved exclusively for the prop-construction sheet, still PENDING-USER-INPUT between a short sword and a spear), regulation steel greaves and deep-brown leather military boots. Materials: matte steel plate, deep-brown leather, plain regulation cloth only — no gemstones, no gold, no jewels. Palette: Hector military standard — steel gray with dark-gold / dark accents, deep-brown leather. Deliberately austere, no decoration — his ordinariness is the point. Character and costume only — no weapon, no handheld items, no standalone props of any kind anywhere on this sheet.

Body metrics:
Height 185 cm. Approximately 7.2 heads tall (design proportion, tall lean build). Shoulder width approximately 2.0 head-widths (leaner than the sturdier member of the pair). Fully symmetric build visually on this zero-prop sheet; his signature weapon (still undecided) will be mounted at the left hip in sheet 09 only.
...
Negative prompt:
photorealistic, 3D render, western comic style, chibi, super-deformed, thick outlines, harsh lineart, sketchy unfinished lines, flat unshaded colors, muddy colors, excessive bloom, dramatic cinematic background, scenery, extreme perspective, fisheye, cropped body, inconsistent anatomy, different face, different hairstyle, costume redesign, random accessories, duplicate character, extra limbs, extra fingers, text, labels, letters, numbers, logo, signature, watermark, jpeg artifacts, low resolution, blurry, heavy hero armor, ornate heroic plate armor, wings, horns, tail, crown, cape, officer sash, command sash, gilded decoration, muscular hulking build, short stocky build, gemstones, ornate gemstones, shining jewels, gold pauldrons, gilded pauldrons, holding weapon, weapon in hand, hand on sword pommel, arms touching torso, arms pressed against body, contrapposto, action pose, heroic pose, battle stance, T-pose, bent elbows, uneven shoulders, crossed legs, hands on hips, crossed arms, dynamic pose, walking, running, cast shadows, strong highlights, rim light, dramatic lighting, weapon, sword, spear, scabbard, sheath, staff, shield, prop object, reference character face, copying any style-image character, blending the three reference characters, Kritz black-and-gold pauldrons, shoulder chains, purple gemstones, Revanas ice-crystal horns, crystalline wings
...
- Do not invent unresolved PENDING-USER-INPUT details (the signature weapon form remains undecided; show only an empty belt mount point, no weapon).
...
## 01 — `01-vincent-front-fullbody.png`（STAGE 2 — 正面全身）
...
Primary request:
Render Vincent as ONE full-body front view (0 degrees) in true orthographic projection, in the standard neutral A-pose: standing upright, feet shoulder-width apart, toes forward, arms straight and relaxed at approximately 30-45 degrees away from the body, palms open with fingers slightly spread and clearly separated, shoulders level, weight evenly distributed, no contrapposto, no action pose, no T-pose.

Identity invariants:
Identical character from Image 1: same plain unremarkable common-soldier face, dark-brown neat military short hair (#2E2015), dark-brown eyes (#3A2818), healthy weathered skin (#E3BB95).

Body metrics:
Height 185 cm. Approximately 7.2 heads tall. Shoulder width approximately 2.0 head-widths. Visually symmetric on this zero-prop sheet.

Costume invariants:
Identical standard-issue Hector infantry armor from Image 1: regulation matte-steel plate pieces (#63676C), plain steel gorget, standard steel shoulder guards, matte-steel breastplate with the small dark-gold Hector insignia (#7A6A42), deep-brown leather bracers (#3D2E22), regulation leather belt with an empty mount point at the left hip and NO weapon attached, regulation steel greaves, deep-brown leather boots. NO weapon anywhere; the signature weapon (still PENDING-USER-INPUT) is reserved exclusively for prop sheet 09.
...
Negative prompt:
photorealistic, 3D render, western comic style, chibi, super-deformed, thick outlines, harsh lineart, sketchy unfinished lines, flat unshaded colors, muddy colors, excessive bloom, dramatic cinematic background, scenery, extreme perspective, fisheye, cropped body, inconsistent anatomy, different face, different hairstyle, costume redesign, random accessories, duplicate character, extra limbs, extra fingers, text, labels, letters, numbers, logo, signature, watermark, jpeg artifacts, low resolution, blurry, heavy hero armor, ornate armor, gemstones, gold pauldrons, wings, horns, tail, officer sash, short stocky build, torch, holding weapon, weapon in hand, weapon, sword, spear, scabbard, sheath, staff, shield, prop object, reference character face, copying any style-image character, arms touching torso, arms pressed against body, contrapposto, action pose, heroic pose, battle stance, T-pose, bent elbows, uneven shoulders, crossed legs, hands on hips, crossed arms, dynamic pose, walking, running, cast shadows, strong highlights, rim light, dramatic lighting, side view, profile, back view, three-quarter view
...
## 02 — `02-vincent-left-profile-fullbody.png`（STAGE 2 — 相機位於角色解剖學左側）
...
Primary request:
Render Vincent as ONE full-body view with the camera positioned at the character's anatomical LEFT side (the camera looks directly at the character's left flank; the character's left side faces the lens), true orthographic projection, 90 degrees from the front view, in the same standard neutral A-pose as the front view: standing upright, feet shoulder-width apart, arms straight and relaxed at approximately 30-45 degrees away from the body, palms open with fingers slightly spread, shoulders level, weight evenly distributed, no contrapposto, no action pose, no T-pose. The character's head faces exactly 90 degrees toward their own anatomical left, away from the camera's direct line; no head turn back toward the lens. Note: his signature weapon's mount point sits at this LEFT hip (per CHARACTER_SPEC.md DESIGN-PROPOSAL) but the weapon itself is still PENDING-USER-INPUT and must NOT appear on this sheet — render the belt with an empty mount point only, exclusively for reference; the weapon itself belongs to sheet 09.

Identity invariants:
Identical character from Image 1: same face structure seen from this angle, dark-brown neat military short hair (#2E2015), healthy weathered skin (#E3BB95).

Body metrics:
Height 185 cm. Approximately 7.2 heads tall. Shoulder width approximately 2.0 head-widths.

Costume invariants:
Identical attire from Image 1 seen from the anatomical-left camera position: matte-steel cuirass silhouette (#63676C), shoulder guard and bracer in profile (#3D2E22), plain regulation belt with an empty mount point and NO weapon attached, steel greaves and boots. NO weapon anywhere; the signature weapon (PENDING-USER-INPUT) is reserved exclusively for prop sheet 09.
...
## 03 — `03-vincent-right-profile-fullbody.png`（STAGE 2 — 相機位於角色解剖學右側）
...
Costume invariants:
Identical attire from Image 1 seen from the anatomical-right camera position: matte-steel cuirass silhouette (#63676C), shoulder guard and bracer in profile (#3D2E22), plain regulation belt with NO weapon attached, steel greaves and boots. NO weapon anywhere; the signature weapon (PENDING-USER-INPUT) is reserved exclusively for prop sheet 09.
...
## 04 — `04-vincent-back-fullbody.png`（STAGE 2 — 背面全身）
...
Primary request:
Render Vincent as ONE full-body BACK view (180 degrees) in true orthographic projection, in the same standard neutral A-pose as the front view: standing upright, feet shoulder-width apart, arms straight and relaxed at approximately 30-45 degrees away from the body, palms open with fingers slightly spread, shoulders level, weight evenly distributed, no contrapposto, no action pose, no T-pose. Head faces exactly away from the viewer.

Identity invariants:
Identical character from Image 1: same dark-brown neat military short hair seen from behind (#2E2015), same build and proportions.

Body metrics:
Height 185 cm. Approximately 7.2 heads tall. Shoulder width approximately 2.0 head-widths.

Costume invariants:
Identical attire from Image 1 seen from behind: plain regulation back plate (#63676C), no ornament, shoulder guards symmetric, regulation belt with an empty mount point at the left hip and NO weapon attached, steel greaves and boots seen from behind. NO weapon anywhere; the signature weapon (PENDING-USER-INPUT) is reserved exclusively for prop sheet 09.
...
## 05 — `05-vincent-four-view-master.png`（STAGE 2 — 四視圖總表）
...
Identity invariants:
All four figures are the identical character from Image 1: same plain unremarkable common-soldier face, dark-brown neat military short hair (#2E2015), dark-brown eyes (#3A2818), healthy weathered skin (#E3BB95). Head-top line and foot line perfectly horizontally aligned across all four views; silhouettes must be mutually alignable.

Body metrics:
Height 185 cm. Approximately 7.2 heads tall, tall lean build. Shoulder width approximately 2.0 head-widths.

Kinship invariants:
none

Costume invariants:
Every view wears the identical standard-issue Hector infantry armor from Image 1: regulation matte-steel plate pieces (#63676C), plain steel gorget, standard steel shoulder guards, matte-steel breastplate with the small dark-gold Hector insignia (#7A6A42), deep-brown leather bracers (#3D2E22), regulation leather belt with an empty mount point at the left hip and NO weapon attached, regulation steel greaves, deep-brown leather boots. Same accessory count, positions, colors and materials in all four views. NO weapon anywhere; the signature weapon (PENDING-USER-INPUT) is reserved exclusively for prop sheet 09.
...
Negative prompt:
photorealistic, 3D render, western comic style, chibi, super-deformed, thick outlines, harsh lineart, sketchy unfinished lines, flat unshaded colors, muddy colors, excessive bloom, dramatic cinematic background, scenery, extreme perspective, fisheye, cropped body, inconsistent anatomy, different face, different hairstyle, costume redesign, random accessories, duplicate character, extra limbs, extra fingers, text, labels, letters, numbers, logo, signature, watermark, jpeg artifacts, low resolution, blurry, heavy hero armor, ornate armor, gemstones, gold pauldrons, wings, horns, tail, officer sash, short stocky build, inconsistent scale between views, different height per view, mismatched costume between views, holding weapon, weapon in hand, weapon, sword, spear, scabbard, sheath, staff, shield, prop object, contrapposto, action pose, T-pose, bent elbows, uneven shoulders, crossed legs, hands on hips, crossed arms, dynamic pose, walking, running, cast shadows, strong highlights, rim light, dramatic lighting, reference character face, copying any style-image character
...
Identity invariants:
Keep exactly the same character in all eight portraits. Preserve the identical facial identity, plain unremarkable soldier-face proportions, healthy weathered skin (#E3BB95), dark-brown eyes (#3A2818), dark-brown neat military short hair (#2E2015), neck proportions, and the visible regulation steel gorget collar. Use the same framing, camera distance, head size, lighting, color grading and front-facing angle in every panel. Only the facial expression may change; his baseline is steady, earnest and unheroic.
...
4. Waist belt detail, including the empty mount point at the LEFT hip reserved for the signature weapon (weapon itself NOT shown here — see sheet 09).
...
Costume invariants:
Exact materials and colors as Image 1: matte steel plate (#63676C), deep-brown leather (#3D2E22), plain regulation cloth (#4A4238). Same construction as Image 1; no redesign; NO weapon anywhere (signature weapon, PENDING-USER-INPUT, belongs to sheet 09 only).
...
- The left-hip belt mount point must be shown EMPTY (no weapon); the weapon itself is PENDING-USER-INPUT and belongs to sheet 09 only.
- No text or watermark. Do not invent missing canonical details.
```

---

## 08 — `08-vincent-color-material-sheet.png`（STAGE 2 — 色票／材質表，已補固定 hex）
...
Primary request:
Create a production color-and-material sheet for Vincent: a medium-size neutral A-pose front figure (character and costume only, no weapon) on the left, and on the right a column of large clean color swatches with material rendering patches for each locked color:
- steel gray #63676C (regulation plate, gorget, greaves)
...
- healthy weathered skin #E3BB95
- dark-brown hair #2E2015
- dark-brown eyes #3A2818
...
Identity invariants:
The reference figure is the identical character from Image 1; same face, hair, proportions (185 cm, 7.2 heads, tall lean).
...
## 09 — `09-vincent-prop-construction-sheet.png`（STAGE 2 — 道具表：短劍〔推薦方案，PENDING-USER-INPUT〕，全書唯一出現位置）

> **PENDING-USER-INPUT — 正典衝突警示**：CHARACTER_SPEC.md 記載簽名武器為「短劍或長矛」二選一，文檔未定案。本提示詞推薦**短劍（regulation short sword）**：長矛屬雙手長柄兵器，與其單兵近戰護衛的敘事定位不完全吻合，且不易與同組角色的短刃裝備並列於 09 道具表；短劍也與同組戴夫、鮑曼的短刃形制一致，便於裝備語言統一。**此為推薦而非定案**，若使用者選擇長矛，僅需重新生成本張（09），00–08／10 不受影響、無需重生成。
...
Primary request:
Create a props-only construction sheet for Vincent's RECOMMENDED signature weapon — a regulation short sword (PENDING-USER-INPUT; the alternative under consideration is a spear, not shown here): full front view, side view, and hilt/guard close-up — matte steel blade (#63676C), plain regulation hilt and guard, deep-brown leather grip (#3D2E22); include its plain regulation scabbard and the belt-hanger hardware sized for the left-hip mount point, as separate callouts.
...
Negative prompt:
photorealistic, 3D render, western comic style, chibi, thick outlines, sketchy lines, muddy colors, dramatic background, scenery, text, labels, letters, numbers, logo, signature, watermark, low resolution, blurry, full-body character, miniature person, portrait, face, human silhouette, character scale inset, hands, arms, extra weapons, twin swords, dual swords, spear, polearm, long shaft weapon, ornate gold decoration, gemstones, neon colors, cast shadows, rim light

Constraints:
- This sheet is the ONLY place the short sword may appear in the entire package.
- Match the described material language exactly (matte steel #63676C, deep-brown leather #3D2E22, plain regulation finish).
- The short sword vs spear choice remains PENDING-USER-INPUT; this sheet renders the recommended short sword. If the user selects the spear instead, only this sheet needs regeneration.
...
Identity invariants:
Same character as Image 1: same face, dark-brown neat military short hair (#2E2015), healthy weathered skin (#E3BB95), dark-brown eyes (#3A2818). The body underneath must be consistent with the armored figure of the approved sheets: tall, plainly proportioned, lean build.

Body metrics:
Height 185 cm. Approximately 7.2 heads tall. Shoulder width approximately 2.0 head-widths. Identical height and proportions in both views; head-top and foot lines horizontally aligned across the two views.
...
- [ ] 髮型、髮色（深棕俐落軍人短髮）一致 #2E2015
- [ ] 身材比例：185cm／約 7.2 頭身、肩寬約 2.0 倍頭寬、高瘦挺拔
- [ ] 服裝結構與 Costume Lock 一致（層次、配件數量、位置）
- [ ] 左右位置正確：佩劍側鎖定為左側腰際（`DESIGN-PROPOSAL`）；00–05／10 零道具無武器可見，僅 07 顯示空掛點、09 顯示武器本身
- [ ] 色票未漂移：#63676C／#7A6A42／#3D2E22／#4A4238／#E3BB95／#2E2015／#3A2818
...
| 張 | 額外驗收 |
| --- | --- |
| 00 | 三視圖齊全（A-pose 正面全身＋正面頭肩＋45° 頭肩），缺一即 REJECT；零道具（無短劍／長矛） |
| 01 | 正面、A-pose、正交 |
...
| 07 | 胸甲拆解面板＋無胸甲軀幹視圖必須存在；左側腰際掛點須顯示為空；無武器本身 |
| 08 | 色票方塊與鎖定 hex 完全一致 |
| 09 | 短劍（推薦方案，`PENDING-USER-INPUT`）唯一出現於此；無角色、無臉、無人物剪影；若使用者改選長矛，僅需重生成本張 |
