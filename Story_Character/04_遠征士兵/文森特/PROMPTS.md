...
第一階段只生成 01-vincent-front-fullbody.png，生成後停止等待使用者核准。核准後才逐次生成下一張；每次仍只生成一張。
...
Canonical character facts (do not add or contradict): BLOCKING SOURCE MISMATCH: the new character master table contains no independently tracked character named Vincent. Do not generate sheet 01 and do not promote any legacy face, costume, weapon, or biography in this package to story canon until the user decides whether Vincent should be retained, renamed, merged into a named character, or absorbed into the Hector-army ensemble.
