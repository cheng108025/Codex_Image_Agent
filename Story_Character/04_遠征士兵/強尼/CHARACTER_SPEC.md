...
> Tier — 配角（遠征誘餌士兵，有名字的犧牲者）。本檔為文字版身份規格（FULL v2 package），供 `00-johnny-character-identity-anchor.png` 與 01–10 全套參考圖生成／校對之依據。
...
## 參考圖片
- 初始角色外觀圖片：無
- 核准後的角色身份正本：`00-johnny-character-identity-anchor.png`（`PENDING-GENERATION`）
...
7. 悲傷 Sadness
8. 眨眼 Wink
> 註：其標誌性情緒為「崇拜」——被點名時因恐懼而發白、最後轉為「遵旨！」的決然。00 號頭肩基準採「望向王的崇拜眼神」為身份標記。表情表僅改變眉眼與嘴部；臉部身份、栗褐色短髮、暖褐眼與制式輕甲跨八格一致。
