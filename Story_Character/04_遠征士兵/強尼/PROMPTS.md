# 強尼·德加 Johnny Dega — Image Generation Prompts（FULL v2 package: 00–10）
...
> **兩階段規則（強制）**：第一階段只生成 `00-johnny-character-identity-anchor.png`（三視圖：A-pose 正面全身＋正面頭肩＋45° 三分之四頭肩，缺一即 REJECT 重生成），生成後停止等待使用者核准。核准後 00 成為 01–10 的 Image 1 身份／服裝唯一正本。三張 style 圖永遠只控制畫風。
...
| 編號 | 檔案 | 內容 |
| --- | --- | --- |
| 00 | `00-johnny-character-identity-anchor.png` | 身份母圖：A-pose 正面全身＋正面頭肩＋45° 頭肩，零道具 |
| 01 | `01-johnny-front-fullbody.png` | 正面全身，A-pose，零道具 |
| 02 | `02-johnny-left-profile-fullbody.png` | 相機位於角色解剖學左側全身，A-pose，零道具 |
| 03 | `03-johnny-right-profile-fullbody.png` | 相機位於角色解剖學右側全身，A-pose，零道具 |
| 04 | `04-johnny-back-fullbody.png` | 背面全身，A-pose，零道具 |
...
| 08 | `08-johnny-color-material-sheet.png` | 色票／材質表（已補固定 hex） |
...
Body metrics:
Height 172 cm. Approximately 7.0 heads tall (design proportion). Shoulder width approximately 2.0 head-widths (slight, not-yet-hardened youthful build). Fully symmetric build, no left-right facial or body asymmetry.
...
## 01 — `01-johnny-front-fullbody.png`（STAGE 2 — 正面全身）
...
Identity invariants:
Identical character from Image 1: same clean beardless boyish face, worshipful warm-brown eyes (#8B5A2B), short neat chestnut-brown hair (#5C3A28), fair skin with a faint flush (#EFCBAE).
...
## 02 — `02-johnny-left-profile-fullbody.png`（STAGE 2 — 相機位於角色解剖學左側）
...
## 03 — `03-johnny-right-profile-fullbody.png`（STAGE 2 — 相機位於角色解剖學右側）
...
## 04 — `04-johnny-back-fullbody.png`（STAGE 2 — 背面全身）
...
## 05 — `05-johnny-four-view-master.png`（STAGE 2 — 四視圖總表）
...
## 08 — `08-johnny-color-material-sheet.png`（STAGE 2 — 色票／材質表，已補固定 hex）
...
- [ ] 色票未漂移：#3C2E24／#4A3B2E／#52565C／#EFCBAE／#5C3A28／#8B5A2B／#E8873A（僅 09）
