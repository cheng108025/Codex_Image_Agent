# 強尼·德加 Johnny Dega — Image Generation Prompts（v2 FULL package: 00 / 05 / 06 / 07 / 08 / 09 / 10）
...
> **兩階段規則（強制）**：第一階段只生成 `00-johnny-character-identity-anchor.png`（三視圖，缺一不可），生成後停止等待使用者核准。核准後 00 成為本檔其餘圖片的 Image 1 身份／服裝唯一正本。三張 style 圖永遠只控制畫風。
...
| 編號 | 檔案 | 內容 |
| --- | --- | --- |
| 00 | `00-johnny-character-identity-anchor.png` | 身份母圖：A-pose 正面全身＋正面頭肩＋45° 三分之四頭肩，零道具 |
| 05 | `05-johnny-four-view-master.png` | 四視圖總表（正面→左→右→背），零道具 |
| 06 | `06-johnny-expression-sheet.png` | 八表情臉部表，4×2 網格 |
| 07 | `07-johnny-costume-detail-sheet.png` | 服裝細節表 |
| 08 | `08-johnny-color-material-sheet.png` | 色票／材質表 |
| 09 | `09-johnny-prop-construction-sheet.png` | 道具表：火炬＋短刀（全書唯一出現位置） |
| 10 | `10-johnny-body-reference-sheet.png` | 素體參考圖（SMPL-X／人體擬合用） |

---

## 00 — `00-johnny-character-identity-anchor.png`（STAGE 1 — 只先生成這張，停止等核准）
...
2. one large straight-on head-and-shoulders portrait carrying his signature worshipful upward gaze;
...
Negative prompt:
photorealistic, 3D render, western comic style, chibi, super-deformed, thick outlines, harsh lineart, sketchy unfinished lines, flat unshaded colors, muddy colors, excessive bloom, dramatic cinematic background, scenery, extreme perspective, fisheye, cropped body, inconsistent anatomy, different face, different hairstyle, costume redesign, random accessories, duplicate character, extra limbs, extra fingers, text, labels, letters, numbers, logo, signature, watermark, jpeg artifacts, low resolution, blurry, heavy plate, full plate armor, heavy armor, ornate armor, noble finery, royal regalia, crown, gemstones, gold pauldrons, wings, horns, tail, beard, stubble, elderly face, fearful expression, angry expression, cynical expression, torch, lit flame, held object, holding weapon, weapon in hand, hand on sword pommel, arms touching torso, arms pressed against body, contrapposto, action pose, heroic pose, battle stance, T-pose, bent elbows, uneven shoulders, crossed legs, hands on hips, crossed arms, dynamic pose, walking, running, cast shadows, strong highlights, rim light, dramatic lighting, weapon, sword, scabbard, sheath, staff, shield, prop object, reference character face, copying any style-image character, blending the three reference characters, Kritz black-and-gold pauldrons, shoulder chains, purple gemstones, Revanas ice-crystal horns, crystalline wings

Constraints:
- The three input images control visual style only; do not reproduce or blend their characters.
- Follow all CANON facts exactly (young beardless clean face, short chestnut-brown hair, worshipful warm-brown eyes, ~172cm slight youthful build, standard light expedition armor).
...
## 05 — `05-johnny-four-view-master.png`（STAGE 2）
...
Input images:
- Image 1: the approved 00-johnny-character-identity-anchor.png and sole identity/costume authority.
...
Identity invariants:
All four figures are the identical character from Image 1: same clean beardless boyish face, short neat chestnut-brown hair, fair skin with a faint flush, warm-brown eyes. Head-top line and foot line perfectly horizontally aligned across all four views; silhouettes must be mutually alignable.

Body metrics:
Height 172 cm. Approximately seven heads tall. Fully symmetric build.
...
## 06 — `06-johnny-expression-sheet.png`（STAGE 2 — 八表情臉部表）
...
Input images:
- Image 1: use only as the character identity, hairstyle, costume-collar and color reference (the approved 00-johnny-character-identity-anchor.png). Ignore its original pose, framing and composition.
...
Identity invariants:
Keep exactly the same character in all eight portraits. Preserve the identical facial identity, clean beardless boyish face proportions, fair skin with a faint flush, warm-brown eyes, short neat chestnut-brown hair, neck proportions, and the visible padded high collar. Use the same framing, camera distance, head size, lighting, color grading and front-facing angle in every panel. Only the facial expression may change; his baseline is a young man carrying a residue of worshipful admiration.
...
## 07 — `07-johnny-costume-detail-sheet.png`（STAGE 2 — 服裝細節表）
...
## 08 — `08-johnny-color-material-sheet.png`（STAGE 2 — 色票／材質表）
...
Primary request:
Create a production color-and-material sheet for Johnny: a medium-size neutral A-pose front figure (character and costume only, no weapon or torch) on the left, and on the right a column of large clean color swatches with material rendering patches for each locked color group, named by color, not hex (CHARACTER_SPEC.md gives no hex values for this character):
...
- fair skin with a faint flush; chestnut-brown hair; warm-brown eyes
...
Identity invariants:
The reference figure is the identical character from Image 1; same face, hair, proportions (172 cm, seven heads).
...
## 09 — `09-johnny-prop-construction-sheet.png`（STAGE 2 — 道具表：火炬與短刀，全書唯一出現位置）
...
Input images:
- Image 1: the approved 00-johnny-character-identity-anchor.png — use ONLY to match material language and craftsmanship level; do not depict the character.
...
## 10 — `10-johnny-body-reference-sheet.png`（STAGE 2 — 素體參考圖，SMPL-X／人體擬合用）
...
Input images:
- Image 1: the approved 00-johnny-character-identity-anchor.png and sole identity authority (face, hair, skin tone, body proportions).
- Images 2-4: shared style references only; do not copy their character identities.
...
Identity invariants:
Same character as Image 1: same face, short neat chestnut-brown hair, fair skin, warm-brown eyes. The body underneath must be consistent with the armored figure of the approved sheets: slight, not-yet-hardened youthful build.

Body metrics:
Height 172 cm. Approximately seven heads tall. Fully symmetric. Identical height and proportions in both views; head-top and foot lines horizontally aligned across the two views.
...
- [ ] 左右位置正確（全對稱設計，無鏡像錯亂）
- [ ] 色票未漂移（暗棕—炭黑皮革／暗鋼灰鋼片／暖褐眼／栗褐髮，對照 CHARACTER_SPEC.md）
...
| 張 | 額外驗收 |
| --- | --- |
| 00 | 三視圖齊全（A-pose 正面全身＋正面頭肩＋45° 頭肩），缺一即 REJECT；零道具（無火炬、無短刀） |
