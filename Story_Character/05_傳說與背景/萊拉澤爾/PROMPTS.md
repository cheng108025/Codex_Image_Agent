# 萊拉澤爾 Lazael — Image Generation Prompts（新版七張制 01–07）
...
> **兩階段規則（強制）**：第一階段只生成 `01-lazael-front-fullbody.png`（三視圖：A-pose 正面全身＋正面頭肩＋45° 三分之四頭肩，缺一即 REJECT 重生成），生成後停止等待使用者核准。核准後 01 成為 02–07 的 Image 1 身份／服裝唯一正本。畫風輸入一律依 `../../STYLE_ANCHOR.md` 決定。
> **檔案安全**：不得刪除或覆蓋現有 PNG；重生成先以新檔名或 `rejects/` 存放，核准後才取代正本。
> **既有 PNG 狀態**：本角色 01–07（非人形 01–06）全部為 `PENDING-GENERATION`。
> **正典備註**：本角色無武器（CANON 非戰鬥者），06 道具結構表放置可選的隨身書卷／古籍（DESIGN-PROPOSAL），沒有武器可移。
...
| 編號 | 檔案 | 內容 |
| --- | --- | --- |
| 01 | `01-lazael-front-fullbody.png` | 身份錨點：正面全身，A-pose，零道具（**STAGE 1，先生成並等核准**） |
| 02 | `02-lazael-four-view-master.png` | 四視圖總表（正面→左→右→背），零道具 |
| 03 | `03-lazael-expression-sheet.png` | 表情表 |
| 04 | `04-lazael-costume-detail-sheet.png` | 服裝細節 |
| 05 | `05-lazael-color-material-sheet.png` | 色彩與材質 |
| 06 | `06-lazael-prop-construction-sheet.png` | 道具結構 |
| 07 | `07-lazael-body-reference-sheet.png` | 身體參考（SMPL-X） |

---

## 01 — `01-lazael-front-fullbody.png`（STAGE 1 — 只先生成這張，停止等核准）
...
Identity invariants:
Per the Character definition above (no prior identity image exists; this sheet establishes it): same bookish gentle clean boyish face, warm brown eyes (#7A5230), soft warm chestnut-brown tousled hair (#8A5A34), fair warm skin (#F2DCC0).
...
Costume invariants:
Identical plain house / casual attire per the Character definition above: soft under-tunic (#F2EAD8) with a plain warm-beige over-shirt or short jacket (#D8C4A0), soft cloth collar with no gorget, natural cloth shoulders with no pauldrons or clasps, plain front with no crest and no armor plates, soft long sleeves with light-brown accents (#B08A5C), simple deep-brown cloth waist sash (#5A4632), knee-length cloth trousers with soft blue-gray accents (#8C97A3), plain soft shoes or short cloth boots. NO weapon, NO armor, NO cape anywhere; any optional accessory belongs exclusively to prop sheet 06.
...
## 02 — `02-lazael-four-view-master.png`（STAGE 2 — 四視圖總表：正面→左→右→背）
...
Input images:
- Image 1: the approved 01-lazael-front-fullbody.png and sole identity/costume authority.
...
## 03 — `03-lazael-expression-sheet.png`（STAGE 2 — 八表情臉部表）
...
Input images:
- Image 1: use only as the character identity, hairstyle, costume-collar and color reference (the approved 01-lazael-front-fullbody.png). Ignore its original pose, framing and composition.
- Image 2: if this role is not the active style anchor, use the approved anchor 01 resolved from `../../STYLE_ANCHOR.md` for style only; otherwise omit Image 2.
...
## 04 — `04-lazael-costume-detail-sheet.png`（STAGE 2 — 服裝細節表，含外衫拆解）
...
## 05 — `05-lazael-color-material-sheet.png`（STAGE 2 — 色票／材質表，固定 hex）
...
## 06 — `06-lazael-prop-construction-sheet.png`（STAGE 2 — 道具表：可選隨身書卷，唯一出現位置）
...
Input images:
- Image 1: the approved 01-lazael-front-fullbody.png — use ONLY to match the soft warm-neutral material language; do not depict the character.
- Image 2: if this role is not the active style anchor, use the approved anchor 01 resolved from `../../STYLE_ANCHOR.md` for style only; otherwise omit Image 2.
...
## 07 — `07-lazael-body-reference-sheet.png`（STAGE 2 — 素體參考圖，SMPL-X／人體擬合用）
...
Input images:
- Image 1: the approved 01-lazael-front-fullbody.png and sole identity authority (face, hair, skin tone, body proportions).
- Image 2: if this role is not the active style anchor, use the approved anchor 01 resolved from `../../STYLE_ANCHOR.md` for style only; otherwise omit Image 2.
...
| 04 | 外衫拆解面板＋無外衫（僅內襯）軀幹視圖必須存在 |
| 05 | 色票方塊與鎖定 hex 完全一致 |
| 06 | 若使用書卷道具，唯一出現於此；無角色、無臉、無人物剪影 |
