...
> Tier S ——《赫克托》聖騎士、克里茲摯友。本檔為文字版身份規格（FULL v2 package），供 `00-gawain-character-identity-anchor.png` 與 01–10 全套參考圖生成／校對之依據。
...
> 服裝版本鎖定：**全盛期聖騎士版**（赫克托銀白板甲＋藍披風＋聖劍＋胸口騎士徽章）。此為 00 與 01–10 的唯一預設外觀；本包不建置其他時期／服裝版本。
...
> 為讓 00 錨定圖完全確定、避免模型隨機決定，以下辨識度關鍵項鎖成具體預設值，寫入 00 的 Character definition。使用者可事後一眼覆蓋。
...
- 本包（`GAWAIN-V1-FULL-GLORY-HOLY-KNIGHT`）之 00–10 一律採未爆發的平靜全盛期狀態，不出現上述兩版本的任何元素；若未來需要，須另開完整 SPEC＋PROMPTS 包，不可在本包內混合或默認切換
...
## 道具與特殊構造（僅出現在 09 道具表）
- 武器：**聖劍 holy sword**——赫克托聖騎士配劍，佩於解剖學右腰（`CANON` 存在；側別 `DESIGN-PROPOSAL`）。**00–05 與 10 一律不出現**
...
- **已決定**：
  ...
  - **側別新增鎖定（本次重寫定案）**：藍披風肩扣固定於解剖學左肩，聖劍固定佩於解剖學右腰；此為既有 PROMPTS.md 已採用的設計提案，本次正式同步回 SPEC，標記 `DESIGN-PROPOSAL`，使用者可覆蓋。
