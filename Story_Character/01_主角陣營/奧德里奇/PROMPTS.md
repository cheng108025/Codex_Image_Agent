...
第一階段只生成 01-aldrich-front-fullbody.png，生成後停止等待使用者核准。核准後才逐次生成下一張；每次仍只生成一張。
...
Canonical character facts (do not add or contradict): Aldrich is Kriz's father, the unnamed Queen's husband, and the central founder-hero figure of the Hector kingdom. In the source range he appears through history, memory, expectation, and political reference rather than confirmed present-tense action. His appearance and current life/death status are not fully established; every concrete face, hair, eye color, age, body metric, costume, crown, armor, or weapon detail remains DESIGN-PROPOSAL and must not be labeled story canon.
