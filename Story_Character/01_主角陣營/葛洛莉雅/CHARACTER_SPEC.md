...
> Tier S ——《赫克托》情感支柱、雙面間諜。本檔為文字版身份規格（FULL v2 package），供 `00-gloria-character-identity-anchor.png` 與 01–10 全套參考圖生成／校對之依據。
...
> **版本切分聲明（稽核 §7 特殊衝突已解決）**：貴族女裝／旅裝主版與潛入用素色斗篷屬不同鏡頭狀態，不得混在同一角色版本包中默認切換。本包鎖定單一主版 `GLORIA-V1-NOBLE-TRAVELWEAR-BADGE`；潛行斗篷版本另立為 `GLORIA-V2-STEALTH-CLOAK`（**本包未建置**，僅記錄版本 ID 供未來需要時另開一包，見文末）。
...
- 本包（`GLORIA-V1-NOBLE-TRAVELWEAR-BADGE`）之 00–10 一律不出現此斗篷；若未來需要此鏡頭狀態，須另開一份完整的 `GLORIA-V2-STEALTH-CLOAK` SPEC＋PROMPTS 包，不可在本包 07／09 內以「替代裝」名義混入
...
## 道具與特殊構造（僅出現在 09 道具表）
- 武器：無（非戰鬥角色）
- 飾品：**高文所贈的承諾徽章**——佩於解剖學左胸，是她與高文情感連結的核心信物（`CANON`）。**00–05 與 10 一律不出現徽章特寫拆解**（徽章本體仍隨服裝出現於 00–05，但拆解細節僅在 09 道具表）
...
- **已決定（本次重寫解決稽核 §7 之衝突）**：
  ...
  - 潛行素色斗篷正式拆分為獨立版本 ID `GLORIA-V2-STEALTH-CLOAK`（未建置），不得作為 09 道具表的「替代裝附件」混入本包，亦不得在本包內任何一張圖跨鏡頭默認切換。
...
- **`PENDING-USER-INPUT`**：無（辨識度關鍵項已於本次重寫全數定案；若使用者未來需要 `GLORIA-V2-STEALTH-CLOAK`，屬新版本開發，非本包待辦）。
