...
> Tier ——《赫克托》配角，執行紀律的冷峻騎士。本檔為文字版身份規格（FULL v2 package），供 `00-albert-character-identity-anchor.png` 與 01–10 全套參考圖生成／校對之依據。
...
- 金屬：拋光深色鋼板甲 **#1C1C1E**；冷鋼劍身 **#8C97A3**（僅 09 出現）；扣件暗金點綴 **#A8863E**（`DESIGN-PROPOSAL`）
...
## 道具與特殊構造（僅出現在 09 道具表）
- 武器：**騎士劍**——決鬥用，動作精確無廢招，佩於解剖學右腰（`CANON` 決鬥；具體劍型與側別 `DESIGN-PROPOSAL`）。**00–05 與 10 一律不出現**
...
- 佩劍側：**解剖學右腰**——本次重寫新增鎖定（原 SPEC 僅要求跨圖一致，未指定側別）。
...
- **已決定**：
  ...
  - **側別新增鎖定（本次重寫定案）**：紀律綬帶／肩章固定於解剖學左肩，騎士劍固定佩於解剖學右腰；此為既有 PROMPTS.md 已採用的設計提案，本次正式同步回 SPEC，標記 `DESIGN-PROPOSAL`，使用者可覆蓋。
  - 全部固定色票已補齊 hex（無既有 PNG 可取樣，依色名推定，標記 `DESIGN-PROPOSAL`，可覆蓋）。
- **`PENDING-USER-INPUT`（僅餘瑣碎項，不影響 00 辨識度）**：
  - 是否配短披風（文檔未載；暫定不配，見 Costume Lock「下襬／披風」）。
  - 佩劍的具體劍型細節（文檔僅載「決鬥」，未給劍型；側別已鎖定右腰，劍型細節不影響身份辨識，留待 09 生成時自由發揮）。
