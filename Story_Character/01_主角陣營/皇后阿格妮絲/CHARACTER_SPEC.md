...
> Tier ——《赫克托》配角，克里茲之母。本檔為文字版身份規格（範本 v2 FULL package），供 `00-agnes-character-identity-anchor.png` 與 01–10 全套參考圖生成／校對之依據。
...
> 服裝版本鎖定：**政變之夜寢房版**（單薄白色寢衣＋褪色金線繡；病弱蜷縮）。此為文檔唯一明載的登場造型。本角色目前無任何已核准 PNG，00 生成前所有非數值型辨識特徵均以下列 `DESIGN-PROPOSAL` 鎖定值為準。
...
- **`PENDING-USER-INPUT`（僅餘瑣碎項，不影響 00 生成）**：
  - 是否需要一個「政變前華貴皇后版」造型（文檔僅載寢房版與「曾經華貴」的過去，未給圖）——另議，不影響本 anchor 辨識度，亦不阻擋本版本 00–10 生成。
