# 伊普拉新 Ipranshin — Image Generation Prompts（v2, STANDARD 7-sheet package: 00 / 05 / 06 / 07 / 08 / 09 / 10）

> **用途聲明（必讀）**
> 這套圖片是 **3D 建模參考包（modeling reference pack）**，不是完成的 3D 模型。
> 純 AI img2vid 無法保證跨鏡頭的角色完全一致；正式影片必須先以本參考包建立同一個經核准的 **Blender／VRM 角色模型**，並以該模型作為**唯一角色正本**驅動所有鏡頭。

> **角色版本 ID：待 SPEC v2 補**（CHARACTER_SPEC.md 尚未列出正式版本 ID 欄位；服裝版本已鎖定為「陰影謀士版」）
...
> **注意**：本檔取代舊版「00–09 by FULL_WORKFLOW_PROMPTS.md 分工」格式，改為本檔自足的 7 表標準包；`FULL_WORKFLOW_PROMPTS.md` 為既有分工文件，不在本次改版範圍內，不得觸碰。
...
| 編號 | 檔案 | 內容 |
| --- | --- | --- |
| 00 | `00-ipranshin-character-identity-anchor.png` | 身份母圖：A-pose 正面全身＋正面頭肩＋45° 三分之四頭肩，零道具，臉須清楚可辨 |
| 05 | `05-ipranshin-four-view-master.png` | 四視圖總表（正面→左→右→背），零道具 |
| 06 | `06-ipranshin-expression-sheet.png` | 八表情臉部表，4×2 網格 |
| 07 | `07-ipranshin-costume-detail-sheet.png` | 服裝細節表（兜帽斗篷拆解＋無斗篷軀幹視圖） |
| 08 | `08-ipranshin-color-material-sheet.png` | 色票／材質表 |
| 09 | `09-ipranshin-prop-construction-sheet.png` | 特殊構造表：神職紋章拆解＋兜帽構造＋袖藏經卷密令機構（無固定武器） |
| 10 | `10-ipranshin-body-reference-sheet.png` | 素體參考圖（SMPL-X／人體擬合用） |

---

## 00 — `00-ipranshin-character-identity-anchor.png`（STAGE 1 — 只先生成這張，停止等核准）
...
## 05 — `05-ipranshin-four-view-master.png`（STAGE 2 — 00 核准後）
...
Identity invariants:
All four figures are the identical character from Image 1: same face, deep calm enigmatic gaze with dark amber eyes, half-lit mood expressed only as a soft one-sided face shadow, sleek near-black hair as seen under the hood on Image 1, fair cool skin, slender seven-heads proportions with the frame concealed by the wide robe. Head-top line and foot line perfectly horizontally aligned across all four views; silhouettes must be mutually alignable.

Kinship invariants:
None.

Body metrics:
Height read ~180cm. Approximately 7.0 heads tall, slender build concealed by the wide robe.

Costume invariants:
Every view wears the identical shadow-schemer attire from Image 1: dark hooded clerical robe with oversized hood pushed back (face fully visible), draped hooded mantle, high covered collar joined to the hood, the single dark-gold clerical sigil on the LEFT chest (fixed side across all views), wide long sleeves with concealed inner storage for scrolls and hidden orders, low understated waist sash, floor-length heavy draped robe and dark soft boots. In every view, both arms are visibly separated from the torso, the sleeves hang naturally from the extended arms without fusing into the robe or mantle, and both relaxed hands remain fully visible beyond the cuffs. Same count, positions, colors and materials in all four views. The back view must clarify how the hooded mantle drapes behind the extended arms and over the back. Character and costume only - no weapons, no handheld items, no standalone props.

Composition:
A single ultra-wide horizontal banner, four full-body figures in ONE row at equal scale and equal size, evenly spaced, none cropped, no second row, flat even ambient lighting with no cast shadows, warm light-gray background, no props beyond the canonical worn robe and concealed sleeve contents, no grid lines, no labels.
...
## 06 — `06-ipranshin-expression-sheet.png`（STAGE 2 — 00 核准後；八表情臉部表）
...
Identity invariants:
Keep exactly the same character in all eight portraits. Preserve the identical facial identity and face proportions, deep dark amber eyes with a calm enigmatic gaze, fair cool skin tone, sleek near-black hair as seen under the hood on Image 1, neck proportions, the oversized hood pushed back and framing (never covering) the face, the high covered clerical collar and the dark-gold clerical sigil trim at the collar. Use the same framing, camera distance, head size, lighting, color grading and front-facing angle in every panel. Only the facial expression may change.
...
Costume invariants:
All details must match Image 1 exactly in count, position, color and material: dark charcoal-to-deep-indigo woven wool robe, matte dark draped mantle, restrained dark-gold clerical sigil on the LEFT chest, minimal dark-metal fittings, no gemstones. Do not redesign, add or remove any element; no weapon anywhere.
...
## 08 — `08-ipranshin-color-material-sheet.png`（STAGE 2 — 00 核准後；色票／材質表）
...
Primary request:
Create a production color-and-material sheet for Ipranshin: a medium-size neutral A-pose front figure (character and costume only, no weapon) on the left, and on the right a column of large clean color swatches with material rendering patches for each locked color: dark charcoal woven wool robe, deep indigo mantle, dark brown accent, restrained dark-gold clerical sigil trim, minimal dark-silver metal fittings, near-black hair, fair cool skin, deep dark amber iris. Each swatch pairs a flat color block with a small material patch showing how that surface reads (matte wool weave, draped mantle fabric, dark-metal fitting, embroidered sigil thread).
...
## 09 — `09-ipranshin-prop-construction-sheet.png`（STAGE 2 — 00 核准後；特殊構造表：無固定武器，改放紋章／構造）
...
1. The dark-gold embroidered circular Tiandi-Orthodox clerical sigil: enlarged flat emblem study showing its internal design.
...
The sigil, hood and sleeve-mechanism studies match the dark charcoal/deep indigo/dark-gold palette on Image 1. Do not add, remove or redesign any element; no weapon of any kind appears anywhere on this sheet.
...
- Match the locked palette exactly (dark charcoal, deep indigo, dark-gold trim).
- No character depiction beyond an optional headless tailor form. No text or watermark.
```

---

## 10 — `10-ipranshin-body-reference-sheet.png`（STAGE 2 — 00 核准後；素體參考圖，SMPL-X／人體擬合用）
...
Identity invariants:
Same character as Image 1: same face, sleek near-black hair, fair cool skin, deep dark amber eyes. The body underneath must be consistent with the robed figure of the approved sheets: slender, previously concealed by the wide robe.

Kinship invariants:
None.

Body metrics:
Height read ~180cm. Approximately 7.0 heads tall, slender build. Identical height and proportions in both views; head-top and foot lines horizontally aligned across the two views.
...
- [ ] 臉型與臉部身份與 00 號正本一致（06–08 的人像面板亦同）；臉清楚可辨、未被兜帽或陰影遮蔽
- [ ] 髮型、髮色（近黑短至中髮）一致；深不可測沉靜眼神一致
- [ ] 身材比例：約 180cm／7.0 頭身／被寬袍遮掩的修長身形
- [ ] 服裝結構與 Costume Lock 一致（兜帽長袍層次、袖藏機構、腰間束帶位置）
- [ ] 左右位置正確：神職紋章固定於左胸，須跨圖一致，未鏡像翻轉
- [ ] 色票未漂移：炭黑／深靛／暗褐主色、暗金神職紋章、深暗琥珀瞳色
