...
> Tier A — 《赫克托》三大貴族之一（商權線）。本檔為文字版身份規格（範本 v2 FULL package），供 `00-adri-character-identity-anchor.png` 與 01–10 全套參考圖生成／校對之依據。
...
> **稽核點名（§3）：本角色為「00 零道具文字不足」名單之一，本版本已修正——00 與所有全身圖（01–05、10）一律明寫「character and costume only — no weapons, no handheld items, no props」；戒指、金鏈與商賈手持道具一律只出現在 09。**
...
## 參考圖片
- 初始角色外觀圖片：無
- 核准後的角色身份正本：`00-adri-character-identity-anchor.png`（`PENDING-GENERATION`）
...
- 標誌性輪廓：華貴長袍的垂墜線條＋滿佩戒指的手部（僅 09）＋不變的微笑（`CANON` 微笑與華服；輪廓構成為 `DESIGN-PROPOSAL`）
- 不可改變特徵：整齊梳髮與修飾短鬚、生意人微笑（笑容與內心不一致）、鳶尾花家紋華貴長袍、戒指與金飾（僅出現於 09）
...
- 袖口／手部：寬袖鑲金邊；**手指佩多枚戒指僅出現於 09 道具表**，00–05／10 全身圖手部不佩戒指
...
- 寶石：戒指鑲寶石（`DESIGN-PROPOSAL`；顏色 `PENDING-USER-INPUT`，僅出現於 09）
- 布料：織錦、絲綢、金線刺繡的高級布料（`CANON` 用料考究；材質細節 `DESIGN-PROPOSAL`）
- 皮革：柔軟皮靴、腰帶（`DESIGN-PROPOSAL`）
- 特殊材質：金飾與寶石的光澤、織錦的暗紋（僅 09）（`DESIGN-PROPOSAL`）
- 膚色／髮色／瞳色（hex）：膚 **#F2DFC8**／髮與短鬚（深棕微灰）**#4A3B34**／瞳 **#8A5A2E**（琥珀褐）（`DESIGN-PROPOSAL` 鎖定）

## 道具與特殊構造（僅出現在 09 道具表）
- 武器：無（商賈非武人）
- 飾品：**多枚戒指與金飾／金鏈**——財富的視覺符號（`CANON`）。**00–05 與 10 一律不出現**
- 隨身道具：帳冊、算籌或華貴摺扇等商賈道具（`DESIGN-PROPOSAL`；預設帳冊，見「鎖定錨定預設值」）。**00–05 與 10 一律不出現**
- 翅膀／角／尾巴／其他特殊構造：無（人類角色）
- 配戴或連接方式：戒指戴於手指、金鏈垂於胸前、腰封束於腰間（僅 09／服裝細節表示意）（`DESIGN-PROPOSAL`）
...
- **不可出現道具的畫面**：00、01–05、10 全身圖一律 character and costume only，禁止戒指、金鏈、帳冊、摺扇、算籌等任何手持或佩掛道具出現；上述道具唯一出現位置為 09
...
- **內袍色**：酒紅 #6E1F2A。
- **金飾**：本角色允許金飾（戒指、金鏈、金扣）為正典財富符號，屬其專屬材質，但**僅限 09 道具表出現**。
- **隨身商賈道具預設**：帳冊（ledger book），摺扇／算籌為可覆蓋替代，僅出現於 09。

## 正典衝突與待確認事項
- **已決定**：
  - 服裝版本採「華貴商賈長袍版」，緊扣「用料考究、佩戒指與金飾、處處顯示財富」的正典描述。
  - 表情基準以「生意人微笑」為預設神情，呼應「表情與內心不一致」的核心設定。
  - 00 與所有全身圖（01–05、10）零道具文字已補全（見「稽核點名」），戒指／金鏈／商賈道具唯一出現於 09。
