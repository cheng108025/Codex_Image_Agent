...
> Tier 配角 — 安德烈之子。本檔為文字版身份規格（範本 v2 FULL package），供 `00-philip-character-identity-anchor.png` 與 01–10 全套參考圖生成／校對之依據。
...
- 左右不對稱特徵：**側劍佩於腰間左側**（`DESIGN-PROPOSAL`，慣用右手拔劍之通用配置，可覆蓋）。因側劍僅出現於 09 道具表，00–05、07、10 全身圖不顯示佩劍或掛點造成的側別差異；09 道具表之劍鞘／腰帶掛點依此左側繪製，供日後合成佩劍鏡頭參照。

## 血緣關係鎖定 Kinship Lock
- 親屬清單：安德烈 `ANDRE-V1-SCHOLAR-ROBE` — 父；賽希莉亞 `CECILIA-V1-STUDY-DOORWAY` — 手足（非雙胞胎，長幼文檔未載）
- 家族錨點：**安德烈**（家族 `ANDRE`）——安德烈的 00 先核准，本角色的 00 才進入生成
...
- 標誌性輪廓：挺拔站姿＋腰間佩劍剪影（僅 09）＋藏不住興奮的上揚眼神（`CANON` 佩劍與興奮；輪廓構成為 `DESIGN-PROPOSAL`）
- 不可改變特徵：男性、英挺面容、興奮發亮的眼神、深色系貴族勁裝、腰間佩劍（僅 09）
- **anatomical left／right 不對稱鎖定**：側劍佩於腰間**左側**（`DESIGN-PROPOSAL`，僅影響 09 道具表繪製；00–05、07、10 全身圖零道具，不顯示佩劍側別）。02 號視圖＝相機位於角色解剖學左側；03 號視圖＝相機位於角色解剖學右側。
...
- 腰部：腰帶佩一柄側劍，掛於左側（`CANON` 佩劍；掛法與側別為 `DESIGN-PROPOSAL`）。**00–05、07、10 全身圖不出現側劍**，僅腰帶本體
- 下襬／披風：勁裝短下襬或及膝外袍，非重甲（`DESIGN-PROPOSAL`）
- 腿部／鞋靴：深色貴族長褲與皮靴（`DESIGN-PROPOSAL`）
- 背面結構：合身勁裝背面，腰帶固定（劍鞘掛點僅示意於 09）（`DESIGN-PROPOSAL`）
...
- 寶石：可省略，或劍柄／釦飾小顆點綴（`DESIGN-PROPOSAL`；顏色 `PENDING-USER-INPUT`，僅出現於 09）
...
## 道具與特殊構造（僅出現在 09 道具表）
- 武器：**側劍 side sword** 一柄，佩於腰間左側（`CANON` 佩劍；側別為 `DESIGN-PROPOSAL`）。**00–05、07、10 一律不出現**
...
- 不可改變的左右位置：佩劍側鎖定左側，須跨圖一致（僅 09 適用）
...
- **已決定**：
  ...
  - 佩劍側鎖定為左側（`DESIGN-PROPOSAL`，可覆蓋），僅影響 09 道具表。
  - 固定色票 hex 值已鎖定（見「色票與材質」），標記 `DESIGN-PROPOSAL`，使用者可覆蓋。
- **已鎖為 00 預設值（`LOCKED`，可覆蓋）**：
