# 菲利浦 Philip — Image Generation Prompts（新版七張制 01–07）
...
> **兩階段規則（強制）**：第一階段只生成 `01-philip-front-fullbody.png`（三視圖：A-pose 正面全身＋正面頭肩＋45° 三分之四頭肩，缺一即 REJECT 重生成），生成後停止等待使用者核准。核准後 01 成為 02–07 的 Image 1 身份／服裝唯一正本。三張 style 圖永遠只控制畫風，不得被當成本角色的身份參考。
> **家族生成順序（§13）**：菲利浦屬於家族 `ANDRE`，錨點為安德烈。**安德烈的 01 必須先核准，本角色的 01 才能進入生成階段。**
> **佩劍側鎖定**：側劍佩於腰間**左側**（`DESIGN-PROPOSAL`，可覆蓋）；因側劍只出現於 06 道具結構表，01–02、07 全身圖一律零道具、不顯示佩劍或掛點側別。
...
| 編號 | 檔案 | 內容 |
| --- | --- | --- |
| 01 | `01-philip-front-fullbody.png` | 身份錨點：正面全身，A-pose，零道具（**STAGE 1，先生成並等核准**） |
| 02 | `02-philip-four-view-master.png` | 四視圖總表（正面→左→右→背），零道具 |
| 03 | `03-philip-expression-sheet.png` | 表情表 |
| 04 | `04-philip-costume-detail-sheet.png` | 服裝細節 |
| 05 | `05-philip-color-material-sheet.png` | 色彩與材質 |
| 06 | `06-philip-prop-construction-sheet.png` | 道具結構 |
| 07 | `07-philip-body-reference-sheet.png` | 身體參考（SMPL-X） |

---

## 01 — `01-philip-front-fullbody.png`（STAGE 1 — 只先生成這張，停止等核准）
...
Identity invariants:
Per the Character definition above (no prior identity image exists; this sheet establishes it): same handsome young noble face, dark brown near-black short hair neatly side-parted (#1C1712), deep amber-brown eyes (#6B4423), fair healthy-toned skin (#F0DCC7).
...
Costume invariants:
Identical dark noble action-attire per the Character definition above: deep blue dark noble palette (#1E2A3F), standing/turned collar with restrained silver-grey embroidery (#8C8C94), subtle Paulownia-house crest on the chest, fitted sleeves, dark noble trousers, leather boots. NO weapon, NO scabbard, NO belt-worn sword anywhere; the side sword is reserved exclusively for prop sheet 06.
...
## 02 — `02-philip-four-view-master.png`（STAGE 2 — 四視圖總表：正面→左→右→背）
...
Input images:
- Image 1: the approved 01-philip-front-fullbody.png and sole identity/costume authority.
...
## 03 — `03-philip-expression-sheet.png`（STAGE 2 — 八表情臉部表）
...
Input images:
- Image 1: use only as the character identity, hairstyle, costume-collar and color reference (the approved 01-philip-front-fullbody.png). Ignore its original pose, framing and composition.
- Image 2: if this role is not the active style anchor, use the approved anchor 01 resolved from `../../STYLE_ANCHOR.md` for style only; otherwise omit Image 2.
...
## 04 — `04-philip-costume-detail-sheet.png`（STAGE 2 — 服裝細節表，含外袍拆解，零武器）
...
4. Waist and belt detail: the plain leather belt (empty of the sword in this view - the sword itself belongs to sheet 06, and its scabbard mounts on the LEFT side).
...
Costume invariants:
Exact colors and materials from Image 1: deep blue dark noble palette (#1E2A3F), restrained silver-grey embroidery (#8C8C94). Same construction as Image 1; no redesign; NO weapon, NO scabbard anywhere (reserved for sheet 06).
...
## 05 — `05-philip-color-material-sheet.png`（STAGE 2 — 色票／材質表，已補固定 hex）
...
## 06 — `06-philip-prop-construction-sheet.png`（STAGE 2 — 道具表：側劍唯一出現位置，佩於左側）
...
Input images:
- Image 1: the approved 01-philip-front-fullbody.png - use ONLY to match material language and craftsmanship level; do not depict the character.
- Image 2: if this role is not the active style anchor, use the approved anchor 01 resolved from `../../STYLE_ANCHOR.md` for style only; otherwise omit Image 2.
...
## 07 — `07-philip-body-reference-sheet.png`（STAGE 2 — 素體參考圖，SMPL-X／人體擬合用）
...
Input images:
- Image 1: the approved 01-philip-front-fullbody.png and sole identity authority (face, hair, skin tone, body proportions).
- Image 2: if this role is not the active style anchor, use the approved anchor 01 resolved from `../../STYLE_ANCHOR.md` for style only; otherwise omit Image 2.
...
- [ ] 臉型與臉部身份與 01 號正本一致（英挺年輕貴公子）
- [ ] 髮型、髮色（深棕近墨黑 #1C1712、側分）一致
- [ ] 身材比例：176cm／約 7.0–7.5 頭身／肩寬約 2.1 倍頭寬
- [ ] 服裝結構與 Costume Lock 一致（深色勁裝層次、家紋位置）
- [ ] 左右位置正確：佩劍側鎖定左側（僅 06 適用），01–02／07 無新增單側特徵、無鏡像錯亂
- [ ] 色票未漂移：#1E2A3F／#8C8C94／#F0DCC7／#1C1712／#6B4423
- [ ] 無任何武器道具出現（06 除外，包含側劍）；平光無投影陰影（03 攝影棚柔光除外）
- [ ] 血緣相似：下頜線與英挺輪廓與家族錨點安德烈的核准 01 可辨識地一致；灰白髮與皺紋（父之特徵）未混淆到本角色
...
| 02 | 順序正面→左→右→背；四視圖同高度、同尺寸、同比例、頭頂線與腳底線水平對齊；相機位於解剖學左側、A-pose、正交（不得只寫「朝左」）、腰帶空無佩劍；相機位於解剖學右側、A-pose、正交（不得只寫「朝右」）、腰帶空無佩劍；背面、A-pose、正交、腰帶空無佩劍 |
| 03 | 恰好 8 格 4×2、順序正確、只有表情改變 |
| 04 | 外層拆解面板＋無外層軀幹視圖必須存在 |
| 05 | 色票方塊與鎖定 hex 完全一致 |
| 06 | 側劍唯一出現於此，佩於左側；無角色、無臉、無人物剪影 |
| 07 | 素體無任何外層與武器；身體輪廓完全可見；正面＋左側兩視圖 |
