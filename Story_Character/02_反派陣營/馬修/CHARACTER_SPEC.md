...
> Tier S — 《赫克托》神權反派。本檔為文字版身份規格（範本 v2 FULL package），供 `00-matthew-character-identity-anchor.png` 與 01–10 全套參考圖生成／校對之依據。
...
>
> ## ⚠️ 服裝版本鎖定聲明（修正稽核 §7 違規，必讀）
> 稽核發現前版套件在同一 07／09 內混入兩個服裝版本（白金神職法袍與戰前皮甲），違反「一個版本 ID 只對應一個故事時期／服裝版本」規則。**本包正式只鎖定 `MATTHEW-V1-CLERIC-ROBE`（白金高階神職法袍，日常／主要辨識形象）一個版本**；00–10 全部只呈現此版本，戰前皮甲版本（`MATTHEW-V2-BATTLE-LEATHER`）已從 07、09 全部移除，本檔文末另立專節註明「未建置」，不得與本包混用。日後若需要皮甲版本，須另開一組完整 00–10（新版本 ID），不得附掛於本包任一張圖內。
...
- 左右不對稱特徵：**無，胸前金墜居中垂於胸口，肩帔為雙肩對稱披覆**（`CANON` 金墜居中；`DESIGN-PROPOSAL` 鎖定為對稱披覆，原文檔僅載「肩帔主披側須跨圖一致」但未指定不對稱側，本次重寫確認為對稱設計，若使用者要求改為不對稱單側批風需另行指定）
...
## 道具與特殊構造（僅出現在 09 道具表）
- 武器：無隨身武器（`DESIGN-PROPOSAL`；本版本不佩武器，00–05 與 10 一律不出現）
...
## 鎖定錨定預設值（可覆蓋）
> 以下辨識度關鍵項已鎖成 00 號具體預設，寫入 00 的 Character definition。使用者可一眼定案後覆蓋任一項，覆蓋後需同步更新 00 及 01–10。
...
## 正典衝突與待確認事項
- **已決定**：
  - 00 號身份基準與 01–10 全部採版本 1 白金高階神職法袍為唯一辨識形象；戰前皮甲版本正式移出本包（見下方「未建置版本」專節）。
...
**`MATTHEW-V2-BATTLE-LEATHER`（戰前皮甲版）** — 文檔載馬修在戰前會褪下法袍改換皮甲（深棕至炭黑皮革胸片、護肩、護臂、腰帶），金墜「月天使」跨版本不變、仍貼身佩戴於胸甲之上。**此版本尚未建置任何 SPEC 或 PROMPTS 內容**；本次重寫已將其從 07（服裝細節表）與 09（道具表）全部移除，修正稽核 §7 指出的「同一套件混入兩個服裝版本」違規。若日後需要此版本出鏡，須另開一組完整 `CHARACTER_SPEC.md`＋`PROMPTS.md`（00–10，版本 ID＝`MATTHEW-V2-BATTLE-LEATHER`），不得附掛於本包（V1）任何一張圖內。
