...
> Tier - 反派陣營／再生型敵人。本檔為文字版身份規格（範本 v2 FULL package），供 `00-adama-character-identity-anchor.png` 與 01–10 全套參考圖生成／校對之依據。
...
## 參考圖片
- 初始角色外觀圖片：無
- 核准後的角色身份正本：`00-adama-character-identity-anchor.png`（`PENDING-GENERATION`；髮型與武器已定案，可依定案值生成）
...
**鎖定為短銀灰髮 short silver-gray hair（#C7CBCE）**（原提案 A，Claude 推薦，使用者已核准）。呼應 CANON 銀灰身軀主色；保留可辨識的「頭髮輪廓」錨點，利於 01–10 identity-preserve 提示詞跨圖鎖定髮型；06 表情表需要穩定的髮際線／髮長參照，短髮風險最低。

> 原 B 案（無髮、結晶化頭部）**未採用**，僅存檔備考：頭皮直接由結晶與裸露肌理構成、無傳統毛髮，理由是更徹底貫徹「非人異形」身體恐怖概念；但因跨圖頭部形狀較難鎖定一致、06 表情表「髮型不變」錨定文字會失效，識別漂移風險較高，故不採用。

### 固定武器（`CANON` — 已核准鎖定，2026-07-23）
**鎖定為無固定武器**（原提案 A，Claude 推薦，使用者已核准）——近戰倚賴自身再生的利爪／結晶化指節，遠程威脅來自其控制的巨龍。與既有敘事定位一致（「威脅來自控龍與再生」）；不需額外新增一件武器道具的完整構造設定，09 道具表專注於墜飾拆解與結晶／肌理構造研究。

> 原 B 案（前臂結晶自體延伸形成的「結晶刃」）**未採用**，僅存檔備考：並非外部持有道具，而是身體結晶生長的一部分，格鬥時自然形成刃狀突起，可賦予更鮮明戰鬥剪影；但需在 09 額外新增一組構造細節，複雜度較高，故不採用。
...
## 正典衝突與待確認事項
- **已決定**：
  - 半透明結晶與裸露再生肌理為核心辨識特徵，所有圖（含 06 表情表、10 素體圖）皆須保留。
  - 頸間銀色控龍墜飾為固定道具（**不包括 10 號素體圖**——見下方 10 號重寫說明，該圖依 §5.5 規則禁止一切 accessories/jewelry）。
  - 主色銀灰＋暗紅；瞳色鎖定暗紅。
  ...
  - **10 號素體圖規格（2026-07-23 修正）**：改為貼身素體衣（tight plain bodysuit），移除銀色墜飾與一切 accessories/jewelry；體表結晶與裸露再生血肉（右前臂與手、左肩與鎖骨、右側肋腹）視為身體解剖構造，保留可見。此修正為符合總規格 §5.5 對第十張素體圖「無鎧甲、無外層服裝、僅著貼身素體衣」及負面提示詞須含 `accessories, jewelry` 的強制規則。
- **`PENDING-USER-INPUT`（不影響辨識度的瑣碎項）**：
  - 墜飾核心是否含發光效果、確切光色（僅影響 00–09；10 號已不含墜飾）。
