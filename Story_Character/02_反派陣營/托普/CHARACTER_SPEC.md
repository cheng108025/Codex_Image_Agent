...
> Tier - 反派陣營／敵方能力者。本檔為文字版身份規格（範本 v2 FULL package），供 `00-top-character-identity-anchor.png` 與 01–10 全套參考圖生成／校對之依據。
...
- 左右不對稱特徵：**短披主扣正式鎖定於角色解剖學左肩**（本次重寫新定案，供跨圖一致性使用；SPEC 原文僅要求「短披主扣側須跨圖一致」未指定側別，此為 `DESIGN-PROPOSAL` 鎖定值，使用者可事後覆蓋，覆蓋後需同步更新 00–10）
...
## 參考圖片
- 初始角色外觀圖片：無
- 核准後的角色身份正本：`00-top-character-identity-anchor.png`（`PENDING-GENERATION`）
...
## 道具與特殊構造（僅出現在 09 道具表）
- 武器：無明確固定武器（`DESIGN-PROPOSAL`；其能力為場地操控而非兵器）。**00–05 與 10 一律不出現**
...
## 鎖定錨定預設值（可覆蓋）
> 以下辨識度關鍵項原為 `PENDING-USER-INPUT`，為生成 00 錨定圖已鎖成具體預設值。使用者可事後一眼覆蓋。
- 髮色：暗炭黑、近黑（tousled boyish 短髮）
- 瞳色：冷銀灰（狹長靈動、帶戲謔笑意的眼）
- 武器：無固定武器（能力為場地操控，非兵器）
- 主要材質：貼身暗色彈性布料＋皮革行動緊身衣＋短披；無硬質甲殼
- 左右不對稱：短披主扣＝左肩（本次重寫新定案）
