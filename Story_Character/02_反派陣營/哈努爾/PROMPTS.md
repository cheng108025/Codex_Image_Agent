# 哈努爾 Hanul — Image Generation Prompts（新版七張制 01–07）
...
> **兩階段規則（強制）**：第一階段只生成 `01-hanul-front-fullbody.png`（三視圖：A-pose 正面全身＋正面頭肩＋45° 三分之四頭肩，缺一即 REJECT 重生成），生成後停止等待使用者核准。核准後 01 成為 02–07 的 Image 1 身份／服裝唯一正本。畫風輸入一律依 `../../STYLE_ANCHOR.md` 決定。
...
> **06 道具結構表調整聲明**：SPEC 明載哈努爾被俘繳械、無武器、無明確固定飾品、無隨身道具。依規則，本輪 06 **不設武器／飾品專表**，改為「破損甲冑五金拆解表」（扣具、綁帶、鉸鏈、破損甲片的構造拆解）。
...
| 編號 | 檔案 | 內容 |
| --- | --- | --- |
| 01 | `01-hanul-front-fullbody.png` | 身份錨點：正面全身，A-pose，零道具（**STAGE 1，先生成並等核准**） |
| 02 | `02-hanul-four-view-master.png` | 四視圖總表（正面→左→右→背），零道具 |
| 03 | `03-hanul-expression-sheet.png` | 表情表 |
| 04 | `04-hanul-costume-detail-sheet.png` | 服裝細節 |
| 05 | `05-hanul-color-material-sheet.png` | 色彩與材質 |
| 06 | `06-hanul-hardware-construction-sheet.png` | 道具結構 |
| 07 | `07-hanul-body-reference-sheet.png` | 身體參考（SMPL-X） |

---

## 01 — `01-hanul-front-fullbody.png`（STAGE 1 — 只先生成這張，停止等核准）
...
Identity invariants:
Per the Character definition above (no prior identity image exists; this sheet establishes it): same rugged weathered face with malicious grin, short coarse dark-brown hair and jaw stubble (#3B2A1E), dark-brown eyes (#3B2416), weathered tan skin (#C79A6E), the left-cheek/jaw gash, wounds and blood at the mouth (#6B1B1B).
...
Costume invariants:
Identical captured-interrogation attire per the Character definition above: battle-damaged military armor (mixed plate #4B4B50 and leather #4A3527), dented LEFT pauldron, damaged collar or exposed wounded neck, torn battle tunic, broken greaves and muddy boots. NO weapon anywhere.
...
## 02 — `02-hanul-four-view-master.png`（STAGE 2 — 四視圖總表：正面→左→右→背）
...
Input images:
- Image 1: the approved 01-hanul-front-fullbody.png and sole identity/costume authority.
...
## 03 — `03-hanul-expression-sheet.png`（STAGE 2 — 八表情臉部表；保留嘴部傷口與鮮血）
...
Input images:
- Image 1: use only as the character identity, hairstyle, costume-collar and color reference (the approved 01-hanul-front-fullbody.png). Ignore its original pose, framing and composition.
- Image 2: if this role is not the active style anchor, use the approved anchor 01 resolved from `../../STYLE_ANCHOR.md` for style only; otherwise omit Image 2.
...
## 04 — `04-hanul-costume-detail-sheet.png`（STAGE 2 — 服裝細節表，含破損甲冑拆解）
...
## 05 — `05-hanul-color-material-sheet.png`（STAGE 2 — 色票／材質表，已補固定 hex）
...
## 06 — `06-hanul-hardware-construction-sheet.png`（STAGE 2 — 破損甲冑五金拆解表；SPEC 無武器無飾品，改以甲冑五金為唯一構造細節）
...
Input images:
- Image 1: the approved 01-hanul-front-fullbody.png - use ONLY to match damaged-metal/leather material language and craftsmanship level; do not depict the character.
- Image 2: if this role is not the active style anchor, use the approved anchor 01 resolved from `../../STYLE_ANCHOR.md` for style only; otherwise omit Image 2.
...
## 07 — `07-hanul-body-reference-sheet.png`（STAGE 2 — 素體參考圖，SMPL-X／人體擬合用）
...
Input images:
- Image 1: the approved 01-hanul-front-fullbody.png and sole identity authority (face, hair, skin tone, body proportions).
- Image 2: if this role is not the active style anchor, use the approved anchor 01 resolved from `../../STYLE_ANCHOR.md` for style only; otherwise omit Image 2.
...
- [ ] 嘴部傷口與鮮血、放肆惡笑跨圖一致；無任何武器道具出現（06 除外，且 06 僅為五金非武器）；平光無投影陰影（03 攝影棚柔光除外）
...
| 02 | 順序正面→左→右→背；四視圖同高度、同尺寸、同比例、頭頂線與腳底線水平對齊；相機位於解剖學左側、A-pose、正交（不得只寫「朝左」）；左頰傷痕與左肩甲破損清楚可辨；相機位於解剖學右側、A-pose、正交（不得只寫「朝右」）；右肩甲相對完好；背面、A-pose、正交 |
| 03 | 恰好 8 格 4×2、順序正確、只有表情改變、嘴部傷口與鮮血每格皆保留 |
| 04 | 胸甲拆解面板＋無胸甲軀幹視圖必須存在 |
| 05 | 色票方塊與鎖定 hex 完全一致 |
| 06 | 破損甲冑五金拆解唯一出現於此（無武器、無飾品）；無角色、無臉、無人物剪影 |
