...
> Tier S — 《赫克托》主要反派。本檔為文字版身份規格（範本 v2 FULL package），供 `00-paranoia-character-identity-anchor.png` 與 01–10 全套參考圖生成／校對之依據。
...
> 服裝版本鎖定：**篡位僭主戰裝版** — 破爛卻僭越華麗的僭主甲冑（像搶來拼湊的王者裝束），配沾血巨劍，指節與頸部爬行電弧。此為與 09 道具表所列 props（沾血巨劍、僭主王冠／甲冑構件、電弧特效研究）最一致的時期，本包不混入其他服裝版本。
...
- 左右不對稱特徵：**不對稱肩甲側正式鎖定為：搶來的華麗鍍金肩甲＝角色解剖學左肩；粗糙實戰甲片＝角色解剖學右肩**（本次重寫新定案，供跨圖一致性使用；SPEC 原文僅載「不對稱的巨大肩甲」未指定側別，此為 `DESIGN-PROPOSAL` 鎖定值，使用者可事後覆蓋，覆蓋後需同步更新 00–10）
...
## 道具與特殊構造（僅出現在 09 道具表）
- 武器：**沾血巨劍 bloody greatsword**——大型雙手巨劍，劍身沾滿乾涸與新鮮血污（`CANON`）。**00–05 與 10 一律不出現**
...
## 鎖定錨定預設值（可覆蓋）
> 依規則：把辨識度關鍵的 `PENDING` 鎖成 00 錨定圖的具體值，避免模型隨機決定。使用者可事後一眼覆蓋。與故事配色（金／銀藍／深色）一致。
...
- **已決定**：
  ...
  - 虹膜、髮色、竊據寶石色、不對稱肩甲側全部正式鎖定（見上表），本次重寫不再保留影響辨識度的 `PENDING-USER-INPUT`。
